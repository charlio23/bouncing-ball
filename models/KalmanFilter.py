...
class KalmanFilter():
    ...
    def _maximization(self):
        ...
        self.A = np.dot(np.sum(self._E_z_z_1,axis=0), np.linalg.inv(np.sum(self._E_z_z[:-1,:,:], axis=0)))
        ...
        self.C = np.dot(np.sum(np.matmul(self.observations.reshape(self.T,self.obs_dim,1), self._E_z.reshape(self.T,1,self.hid_dim)),axis=0), np.linalg.inv(np.sum(self._E_z_z, axis=0)))
    ...
    def filter(self):
        ...
        for t in range(self.T):
            if t == 0:
                K = np.dot(np.dot(self.P_0,self.C.T), np.linalg.inv(np.dot(np.dot(self.C,self.P_0),self.C.T) + self.cov_2))
    ...
    def smooth(self):
        ...
        for i in range(self.T):
            t = self.T - 1 - i
            self.J[t,:,:] = np.dot(np.dot(self.filtered_state_cov[t,:,:], self.A.T), np.linalg.inv(self.P[t,:,:]))
            if t == (self.T-1):
    ...
    def compute_loglikelihood(self):
        ...
        E_0 = np.trace(np.dot(np.linalg.inv(self.P_0), self.smoothed_state_cov[0,:,:])) 
        ...
        inv_cov_2 = np.linalg.inv(self.cov_2)
